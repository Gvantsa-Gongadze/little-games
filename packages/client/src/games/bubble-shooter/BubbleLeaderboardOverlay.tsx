...
export function BubbleLeaderboardOverlay({ score, state, onRestart }: Props) {
  ...
  useEffect(() => {
    ...
    getLeaderboard('bubble-shooter', 10)
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        boxShadow: `0 0 24px ${PINK}44, inset 0 0 40px rgba(0,0,0,0.6)`,
        background: '#07040f',
        color: PINK,
        textAlign: 'center',
      }}>
        <div style={{ fontSize: 20, letterSpacing: 4, marginBottom: 6 }}>
          {state === 'won' ? T.bubble.win : T.bubble.gameOver}
        </div>

        <div style={{ fontSize: 11, letterSpacing: 3, color: YELLOW, marginBottom: 24 }}>
          {T.bubble.score} &nbsp; {String(score).padStart(6, '0')}
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 24 }}>
            <tbody>
              {entries.map((e, i) => {
                ...
                return (
                  <tr key={i} style={{ color: isYou ? YELLOW : PINK }}>
        ...
        <button
          ...
          style={{
            ...
            color: PINK,
            cursor: 'pointer',
            boxShadow: `0 0 10px ${PINK}44`,
            transition: 'background 0.15s',
          }}
          onMouseEnter={e => (e.currentTarget.style.background = '#2a0820')}
